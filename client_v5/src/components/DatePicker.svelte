<script lang="ts">
	...
	let selected: luxon.DateTime = DateTime.fromJSDate(current);
	/**
...
<template>
	<div class="relative">
		{#if disabled}
			<span class="btn-disabled">{selected.toFormat($_('adapters.date_format'))}</span>
		{:else}
			<button on:click={toggleCalendar} class="btn" {id} role="button">
				{selected.toFormat($_('adapters.date_format'))}
			</button>
		{/if}
		{#if showDatePicker || keepOpen}
			<div class={keepOpen ? 'permanent' : 'popup'}>
				<div class="month-name">
					...
					<div class="center">{selected.month} {selected.year}</div>
					<div class="center">
