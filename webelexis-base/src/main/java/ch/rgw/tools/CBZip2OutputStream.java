...
/**
 * An output stream that compresses into the BZip2 format (without the file header chars) into
 * another stream. TODO: Update to BZip2 1.0.1
 * 
...
public class CBZip2OutputStream extends OutputStream implements BZip2Constants {
	...
	/*
	 * If you are ever unlucky/improbable enough to get a stack overflow whilst sorting, increase
	 * the following constant and try again. In practice I have never seen the stack go above 27
	...
	private void allocateCompressStructures(){
		...
		/*
		 * The back end needs a place to store the MTF values whilst it calculates the coding
		 * tables. We could put them in the zptr array. However, these values will fit in a short,
		 * so we overlay szptr at the start of zptr, in the hope of reducing the number of cache
		 * misses induced by the multiple traversals of the MTF values when calculating coding
		 * tables. Seems to improve compression speed by about 1%.
		 */
		// szptr = zptr;
	...
	private void endBlock() throws IOException{
		 ...
		 * does not really give a strong enough guarantee that the value will not appear by chance
		 * in the compressed datastream. Worst-case probability of this event, for a 900k block, is
		 * about 2.0e-3 for 32 bits, 1.0e-5 for 40 bits and 4.0e-8 for 48 bits. For a compressed
		 * file of size 100Gb -- about 100000 blocks -- only a 48-bit marker will do. NB: normal
		 * compression/ decompression do *not* rely on these statistical properties. They are only
	...
	private void initialize() throws IOException{
		/*
		 * Write `magic' bytes h indicating file-format == huffmanised, followed by a digit
	...
	private void mainSort(){
		...
		if (m_last < 4000) {
			...
			for (i = 0; i <= 255; i++) {
				 ...
				 * Hopefully previous pointer-scanning phases have already completed many of the
				 * small buckets [ss, j], so we don't have to sort them at all.
				 */
				for (j = 0; j <= 255; j++) {
					sb = (ss << 8) + j;
				 ...
				 * necessary. The "if (i < 255)" test merely skips this updating for the last bucket
				 * processed, since updating for the last bucket is pointless.
				 */
				bigDone[ss] = true;
	...
	private void qSort3(int loSt, int hiSt, int dSt){
		...
		while (sp > 0) {
			...
			med =
				med3(m_block[m_zptr[lo] + d + 1], m_block[m_zptr[hi] + d + 1],
