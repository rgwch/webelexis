            ...
            if (session != null) {
                ...
                findUser(username, new Handler<Message<JsonObject>>() {
                    ...
                    public void handle(Message<JsonObject> findResult) {
                        ...
                        JsonObject user = findResult.body().getJsonObject("result");

                        if (cfg.getMandatoryString("status").equals("ok")) {
                            if (user == null) {
                            ...
                            log.severe(findResult.body().encodePrettily());
        ...
        if (user != null) {
            findUser(user.getString("username"), new Handler<Message<JsonObject>>() {
                ...
                public void handle(Message<JsonObject> dbReply) {
                    log.finest("mongo result: " + dbReply.body().encodePrettily());

                    if (cfg.getMandatoryString("status", dbReply).equals("ok")) {
    ...
    private boolean checkPwd(JsonObject user, String pwdToCheck) {
        if (user.getBinary("pwhash") == null) {
            ...
            eb.send(persistorAddress, new JsonObject().put("action", "update").put(
                    ...
                    .put("objNew", user).put("upsert", false).putBoolean("multi", false));
        }
