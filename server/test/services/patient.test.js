...
describe('\'patient\' service', () => {
  ...
  it("fetches all patients born in 1990", async ()=>{
    return service.find({query: {$find: "1990"}}).then(result=>{
      result.should.be.ok
      result.data.every(elem=>elem.geburtsdatum.startsWith("1990")).should.be.true
    })
  })
  it("fetches all patients with birthday at 14th of february", ()=>{
    return service.find({query:{$find: "14.2."}}).then(result=>{
      result.should.be.ok
